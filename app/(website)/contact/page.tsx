...
export default function Contact() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim())
      errors.name = t("contact.nameRequired", "Name is required");
    if (!formData.subject.trim())
      errors.subject = t("contact.subjectRequired", "Subject is required");
    if (!formData.inquiryType)
      errors.inquiryType = t(
        "contact.inquiryTypeRequired",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/contact/submit`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        if (result.details && Array.isArray(result.details)) {
          ...
          setSubmitStatus({
            type: "error",
            message:
              result.error || result.message || t("contact.errorMessage"),
          });
        }
      }
    } catch (error) {
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {t("contact.emailHelp", "Enter a valid email address")}
                      </p>
                    </div>
                  </div>
                        ...
                        <SelectContent>
                          {inquiryTypes.map((type) => (
                            <SelectItem key={type} value={type}>
                              {
                                inquiryTypeLabels[
                                  type as keyof typeof inquiryTypeLabels
                                ]
                              }
                            </SelectItem>
                          ))}
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger className="mt-1">
                        <SelectValue
                          placeholder={t(
                            "contact.preferredContactPlaceholder",
                  ...
                  <div>
                    ...
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <p
                        className={`text-xs ml-auto ${formData.message.length < 10 ? "text-red-500" : formData.message.length > 1800 ? "text-orange-500" : "text-gray-500"}`}
                      >
                ...
                <CardContent>
                  ...
                  <div className="mt-4 p-3 bg-orange-50 rounded-lg">
                    <p className="text-sm text-orange-800">
                      <strong>
                        {t(
                          "contact.emergencyConsultations",
                          "Emergency Consultations:"
                        )}
                      </strong>{" "}
                      {t(
                        "contact.emergencyConsultationsText",
                        "Available 24/7 for urgent matters. Additional charges may apply for after-hours consultations."
                      )}
                    </p>
                  </div>
                </CardContent>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <Card className="overflow-hidden">
            <div className="w-full">
              <iframe
                ...
                title="Astrology Center Location"
                className="w-full"
              ></iframe>
            </div>
          </Card>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600">
              <strong>Astrology Center</strong>
              <br />
              New Delhi, India
            </p>
          </div>
